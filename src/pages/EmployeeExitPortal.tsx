...
type DeptKey = 'itClr' | 'financeClr' | 'hrClr' | 'mgrClr';
type LiveStatus = {
  overall_status: string;
  departments: Record<DeptKey, { label: string; cleared: boolean; text: string }>;
...
export default function EmployeeExitPortal() {
  ...
  const [agreementCb, setAgreementCb] = useState(false);
  ...
  const validate = (step: number) => {
    ...
    if (step === 2) {
      if (!agreementCb) { newErrors.agreementCb = true; ok = false; }
    }
    if (step === 3) {
  ...
  const downloadPDF = () => {
    ...
    // ── Header band ──
    doc.setFillColor(232, 81, 10); // brand orange
    doc.rect(0, 0, pageWidth, 30, 'F');
    doc.setTextColor(255, 255, 255);
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(18);
    doc.text('Employee Exit Summary', 14, 18);
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(10);
    doc.text('Magsmen Strategy Consultants — Grofesion Innovations Pvt Ltd', 14, 25);
    doc.setTextColor(0, 0, 0);

    const tableDefaults = {
      theme: 'grid' as const,
      styles: { fontSize: 10, cellPadding: 4, valign: 'middle' as const },
      headStyles: { fillColor: [28, 28, 28] as [number, number, number], textColor: 255, fontStyle: 'bold' as const, fontSize: 11 },
    ...
    autoTable(doc, {
      ...tableDefaults,
      startY: y,
      head: [['RESIGNATION DETAILS', '']],
      body: [
        ['Full Name', `${vals.firstName} ${vals.lastName}`],
        ['Employee ID', vals.employeeId],
        ['Email', vals.email],
        ['Department', vals.department],
        ['Designation', vals.designation],
        ['Reporting Manager', vals.manager],
        ['Last Working Day', vals.lastWorkingDay],
        ['Notice Period', vals.noticePeriod],
        ['Reason for Leaving', vals.reason],
        ['Additional Context', vals.reasonDetail || '—'],
      ],
    });
    ...
    autoTable(doc, {
      ...
      head: [['EXIT AGREEMENT', '']],
      body: [
        ['Confidentiality / Non-Disparagement / Non-Solicitation / Company Property Terms', agreementCb ? 'Accepted' : 'Not Accepted'],
      ],
    });
    y = (doc as any).lastAutoTable.finalY + 8;
    ...
    autoTable(doc, {
      ...tableDefaults,
      startY: y,
      head: [['HANDOVER CHECKLIST', 'STATUS']],
      body: handoverItems.map(h => [h.title, readHandover.has(h.id) ? 'Completed' : 'Not Completed']),
    ...
    autoTable(doc, {
      ...tableDefaults,
      startY: y,
      head: [['ASSET RETURN', 'STATUS']],
      body: assetItems.map(a => [a.label, assetsReturned[a.id] ? 'Returned' : 'Not Returned']),
      columnStyles: { 0: { cellWidth: 130 } },
    });
    y = (doc as any).lastAutoTable.finalY + 8;
    ...
    autoTable(doc, {
      ...tableDefaults,
      startY: y,
      head: [['NO-DUES DECLARATION', 'STATUS']],
      body: clearanceItems.map(c => [c.dept, clearanceCb[c.id] ? 'Declared' : 'Not Declared']),
      columnStyles: { 0: { cellWidth: 130 } },
    });
    y = (doc as any).lastAutoTable.finalY + 8;
    ...
    autoTable(doc, {
      ...tableDefaults,
      startY: y,
      head: [['EXIT INTERVIEW — RATINGS', 'SCORE']],
      body: ratingFields.map(r => [r.label, `${ratings[r.id] || 0} / 5`]),
      columnStyles: { 0: { cellWidth: 130 } },
    });
    y = (doc as any).lastAutoTable.finalY + 8;
    ...
    autoTable(doc, {
      ...tableDefaults,
      startY: y,
      head: [['EXIT INTERVIEW — FEEDBACK', '']],
      body: [
        ['What they liked most', vals.likeMost || '—'],
        ['What could improve', vals.improve || '—'],
        ['Would recommend Grofesion', recommend || '—'],
        ['Would consider rejoining', rejoin || '—'],
      ],
    });
    y = (doc as any).lastAutoTable.finalY + 8;
    ...
    autoTable(doc, {
      ...tableDefaults,
      startY: y,
      head: [['DECLARATION & SIGNATURE', '']],
      headStyles: { ...tableDefaults.headStyles, fillColor: [232, 81, 10] as [number, number, number] },
      body: [
        ['Digital Signature', vals.signature],
        ['Date of Signing', vals.signDate],
      ],
    });
    ...
    doc.save(`Exit_Summary_${vals.firstName}_${vals.employeeId || ''}.pdf`);
  };
  ...
  const submitExit = async () => {
    ...
    try {
      ...
      formData.append("agreementCb", String(agreementCb));
  ...
  return (
    <>
          ...
          <div className="prog-steps">
            {[1, 2, 3, 4, 5, 6, 7].map(s => (
              <div key={s} className={`ps ${currentStep === s ? 'active' : (currentStep as number) > s ? 'done' : ''}`}>
                ...
                <div className="ps-lbl">{['Resignation', 'Agreement', 'Handover', 'Assets', 'Clearance', 'Exit Interview', 'Sign & Submit'][s - 1]}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="shell">
        ...
        {currentStep === 1 && (
          <div className="step active">
            <div className="hero">
              ...
              <div className="hero-body">This portal walks you through your exit agreement, handover, asset return, department clearance, and your exit interview. Takes about 15 minutes — please complete all steps before your last working day.</div>
              <div className="hero-meta">
                <div className="hm-item"><div className="hm-lbl">Entity</div><div className="hm-val">Magsmen Strategy Consultants</div></div>
            ...
            <div className="step-eye">Step 1 of 7 — Resignation Details</div>
                ...
                <div className="field">
                  ...
                  {errors.employeeId && <div className="field-error" style={{ display: 'block' }}>Required.</div>}
                      ...
                      <div className="btn-row">
                          <button className="btn-primary" onClick={() => goTo(2)}>Continue → Exit Agreement</button>
                      </div>
                  </div>
              )}

        {/* STEP 2 — EXIT AGREEMENT */}
        {currentStep === 2 && (
          <div className="step active">
            <div className="step-eye">Step 2 of 7 — Exit Agreement</div>
            <div className="step-title">Review & Accept Your Resignation / Exit Agreement</div>
            <div className="step-sub">This is a legally binding document covering confidentiality, non-disparagement, non-solicitation, and return of company property. Read it fully before accepting.</div>

            <div className="card">
              <div className="card-head">Resignation / Exit Agreement</div>
              <div className={`doc-read-note ${scrolledAgreement ? 'done' : ''}`}>
                {scrolledAgreement ? '✓ You have read the full document' : '📖 Scroll to read the full agreement before accepting'}
              </div>
              <div className="doc-scroll" onScroll={checkScroll}>
                <p><strong>Magsmen Strategy Consultants</strong> and <strong>{vals.firstName} {vals.lastName}</strong> hereby agree to this Resignation/Exit Agreement effective <strong>{vals.lastWorkingDay || 'your last working day'}</strong>. As a reminder, the Employer's non-disclosure and non-distribution agreements are excepted below.</p>
                <p>The Employee and the Employer were governed by an employment agreement and mutually agreed to the terms and conditions of employment during the course of engagement.</p>
                <p>Based on the Employee's communication regarding separation, the Employer has processed the Employee's exit in accordance with internal policies and applicable procedures.</p>

                <h4>Confidentiality and Non-Disclosure</h4>
                <p>The Employee agrees that they shall not disclose, distribute, publish, or communicate, in any format or forum, any information relating to the Employer or its customers, vendors, owners, shareholders, employees, partners, officers, directors, board members, or affiliated companies that is confidential or considered a trade secret.</p>
                <p>This includes, but is not limited to, information relating to patents, copyrights, trademarks, service marks, trade names, business processes, strategies, projects, products, or any intellectual property invented, developed, or worked upon by the Employer or the Employee during the course of employment.</p>

                <h4>Non-Disparagement</h4>
                <p>The Employee agrees not to make any statements relating to their employment or this document that may be construed as libelous, slanderous, defamatory, critical, or otherwise derogatory toward the Employer or its employees, agents, partners, shareholders, officers, directors, board members, or affiliated companies.</p>

                <h4>Non-Solicitation</h4>
                <p>During the term of employment and for a period of one (1) year following the Employee's separation date, the Employee shall not, directly or indirectly, for personal benefit or on behalf of any third party other than the Employer, engage in any activity that may cause or attempt to cause any employee, vendor, contractor, consultant, or other agent of the Employer to terminate or disrupt their business relationship with the Employer.</p>

                <h4>Company Property and Access to Company Resources</h4>
                <p>The Employee certifies that all company property has been returned to the Employer, including but not limited to uniforms, laptops, mobile devices, pen drives, documents, creative files, login credentials, project documents, and any physical or electronic materials or intellectual property belonging to the Employer.</p>
                <p>The Employee further confirms that they do not retain access to any Employer-owned systems, servers, accounts, subscriptions, or other digital resources and have discontinued the use of such resources on any personal or home devices unless expressly authorized in writing by the Employer.</p>

                <h4>Confidentiality, Dues, and Legal Remedies</h4>
                <p>The Employer shall pay the Employee any outstanding approved dues, if applicable, in accordance with company policy.</p>
                <p>In the event of any violation of the terms stated herein, the aggrieved party shall have the right to pursue appropriate legal remedies available under applicable law, including injunctive relief and/or recovery of damages.</p>
              </div>
              <div className="field">
                <div className="cb-row" onClick={() => { setAgreementCb(!agreementCb); if (errors.agreementCb) setErrors(prev => ({ ...prev, agreementCb: false })); }}>
                  <input type="checkbox" checked={agreementCb} readOnly />
                  <div>
                    <div className="cb-row-text">I have read and understood the Resignation/Exit Agreement in its entirety and I accept all terms and conditions stated above, including confidentiality, non-disparagement, and non-solicitation obligations that survive my employment.</div>
                    <div className="cb-row-sub">Date of acceptance: <span>{vals.signDate}</span></div>
                  </div>
                </div>
                {errors.agreementCb && <div className="field-error" style={{ display: 'block' }}>Please read and accept the Exit Agreement to continue.</div>}
              </div>
            </div>

            <div className="btn-row">
              <button className="btn-secondary" onClick={() => goTo(1)}>← Back</button>
              <button className="btn-primary" onClick={() => goTo(3)}>Continue → Handover</button>
            </div>
          </div>
        )}
        ...
        {currentStep === 3 && (
          <div className="step active">
            <div className="step-eye">Step 3 of 7 — Handover</div>
            ...
            <div className="alert alert-warn"><div className="alert-icon">⚠️</div><div>Clearance in Step 5 cannot be requested until every required handover item below is marked complete.</div></div>
            ...
            <div className="btn-row">
              ...
              <button className="btn-primary" onClick={() => goTo(4)}>Continue → Assets</button>
            </div>
          </div>
        )}
        ...
        {currentStep === 4 && (
          <div className="step active">
            <div className="step-eye">Step 4 of 7 — Asset Return</div>
            ...
            <div className="btn-row">
              ...
              <button className="btn-primary" onClick={() => goTo(5)}>Continue → Clearance</button>
            </div>
          </div>
        )}
        ...
        {currentStep === 5 && (
          <div className="step active">
            <div className="step-eye">Step 5 of 7 — Clearance</div>
            ...
            <div className="btn-row">
              ...
              <button className="btn-primary" onClick={() => goTo(6)}>Continue → Exit Interview</button>
            </div>
          </div>
        )}
        ...
        {currentStep === 6 && (
          <div className="step active">
            <div className="step-eye">Step 6 of 7 — Exit Interview</div>
        ...
        {currentStep === 7 && (
          <div className="step active">
            ...
            <div className="card">
              ...
              <div className="sum-grid">
                {[
                  ...
                  { label: 'Exit Agreement', value: agreementCb ? 'Accepted' : '—' },
            ...
            <div className="card">
              ...
              <div className="decl">I, <strong>{vals.firstName} {vals.lastName}</strong>, confirm that I have read and accepted the Resignation/Exit Agreement, completed my handover, returned all company assets, submitted my no-dues declarations, and provided honest exit interview feedback. I understand that my full and final settlement, relieving letter, and experience certificate will be processed as per the <strong>Grofesion Employee Handbook</strong> and remain subject to department clearance.</div>
              <div className="field">
          ...
          <div className="step active">
            <div className="success-wrap">
              ...
              <div className="s-checks">
                ...
                <div className="sck"><div className="sck-dot">✓</div><span>Exit Agreement accepted</span></div>
...
/* DOCUMENT VIEWER (Exit Agreement) */
.doc-scroll{background:var(--gray-100);border:1px solid var(--gray-200);border-radius:var(--r-sm);padding:20px;max-height:280px;overflow-y:auto;font-size:13px;color:var(--gray-600);line-height:1.75;margin-bottom:14px}
...
//     doc.text(`Exit Agreement Accepted: ${agreementCb ? 'Yes' : 'No'}`, 20, 80);
//     doc.text(`Signed: ${vals.signature} on ${vals.signDate}`, 20, 90);
